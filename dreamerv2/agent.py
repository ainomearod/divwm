...
class Agent(common.Module):
    def __init__(self, config, obs_space, act_space, step):
        ...
        if config.task in ['dmc_walker_all', 'dmc_walker_walk', 'dmc_cheetah_all']:
            self._task_behavior = {
                key: ActorCritic(config, self.act_space, self.tfstep)
                for key in DMC_TASK_IDS[config.task]
            }
        else:
    ...
    def re_init_explorers(self):
        self._expl_behavior = expl.Plan2Explore(self.config, self.act_space, self.wm, self.tfstep,
                                                lambda seq: self.wm.heads['reward'](seq['feat']).mode())
    ...
    def train(self, data, state=None):
        ...
        if self.config.expl_behavior != 'greedy':
            ...
            print("trained the explorers", flush=True)
    ...
    @tf.function
    def model_reward_train(self, data):
        _, _, _, metrics = self.wm.train_reward_heads(data)
        return metrics

    @tf.function
    def agent_train_eval(self, data, goal=''):
        data = self.wm.preprocess(data)
        embed = self.wm.encoder(data)
        start, _ = self.wm.rssm.observe(embed, data['action'], data['is_first'])

        reward = lambda seq: (self.penalized_reward(seq, goal=goal), {})
        if goal == '':
            metrics = self._task_behavior.train(self.wm, start, data['is_terminal'], reward)
        else:
            metrics = self._task_behavior[goal].train(self.wm, start, data['is_terminal'], reward)
        if self.config.offline_tune_lmbd:
    ...
    @tf.function
    def agent_train_expl(self, data, state=None):
        data = self.wm.preprocess(data)
        embed = self.wm.encoder(data)
        start, _ = self.wm.rssm.observe(embed, data['action'], data['is_first'])
        _, _, outputs, _ = self.wm.loss(data, state)
    ...
    @tf.function
    def calibrate_lmbd(self, data):
        if self.config.offline_tune_lmbd:
            data = self.wm.preprocess(data)
            embed = self.wm.encoder(data)
            start, _ = self.wm.rssm.observe(embed, data['action'], data['is_first'])
            seq = self.wm.imagine(self._task_behavior.actor, start, data['is_terminal'], self.config.imag_horizon)
            pen = self.compute_penalty(seq)
            lmbd = self.config.offline_lmbd_cons / tf.math.reduce_mean(pen)
            log_lmbd = tf.clip_by_value(tf.math.log(lmbd), -5, 5)
            self.log_lmbd.assign(log_lmbd)
            print('Calibrated lambda!')
        else:
            print('Not tuning lambda!')
    ...
    @tf.function
    def penalized_reward(self, seq, goal=''):
        if goal == '':
            rew = self.wm.heads['reward'](seq['feat']).mode()
        else:
            rew = self.wm.heads['reward_'+goal](seq['feat']).mode()
...
class WorldModel(common.Module):
    def __init__(self, config, obs_space, tfstep):
        ...
        if config.task in ['dmc_walker_all', 'dmc_walker_walk', 'dmc_cheetah_all']:
            self.heads.update({f'reward_{DMC_TASK_IDS[config.task][idx]}': common.MLP([], **config.reward_head)
                               for idx in range(len(DMC_TASK_IDS[config.task]))})
        else:
    ...
    def train_reward_heads(self, data, state=None):
        ...
        with tf.GradientTape() as model_tape:
            likes = {}
            losses = {}
            if self.config.task in DMC_TASK_IDS.keys():
                for idx, goal in enumerate(DMC_TASK_IDS[self.config.task]):
                    name = 'reward_' + goal
                    grad_head = True
                    inp = feat if grad_head else tf.stop_gradient(feat)
                    out = self.heads[name](inp)
                    like = tf.cast(out.log_prob(data['reward'][:, :, idx]), tf.float32)
                    likes[name] = like
                ...
                inp = feat
                out = self.heads[name](inp)
                like = tf.cast(out.log_prob(data['reward']), tf.float32)
                likes[name] = like
                losses[name] = -like.mean()
            model_loss = sum(v for k, v in losses.items())
        metrics = {f'{name}_loss': value for name, value in losses.items()}
        last_state = {k: v[:, -1] for k, v in post.items()}
        modules = [self.heads[name]]
        ...
        for name, head in self.heads.items():
            ...
            for key, dist in dists.items():
                if 'reward_' in key:
                    ...
                    rew_idx = DMC_TASK_IDS[self.config.task].index(rew_key)
...
class MultiHeadActorCritic(ActorCritic):
    ## agents have the same feature extraction layers, but diff heads
    ## both the actor and critic have multiple heads
    ## agents share an optimizer, with the loss = sum_i L_i
    ## agents have their own reward norm, so they can be treated equally
    def __init__(self, config, act_space, tfstep):
        self.config = config
        self.act_space = act_space
        self.tfstep = tfstep
        ...
        if self.config.actor_grad == 'auto':
            self.config = self.config.update({'actor_grad': 'reinforce'})
        self.actor = common.MultiMLP(act_space.shape[0], **self.config.actor)
        self.critic = common.MultiMLP([], **self.config.critic)
        if self.config.slow_target:
            self._target_critic = common.MultiMLP([], **self.config.critic)
            self._updates = tf.Variable(0, tf.int64)
        else:
            self._target_critic = self.critic
        self.actor_opt = common.Optimizer('actor', **self.config.actor_opt)
    ...
    def train(self, world_model, start, is_terminal, reward_fn):
        ...
        for i in range(self.num_agents):
            seqs += [world_model.imagine(self.actor, start, is_terminal, hor, idx=i)]
        ...
        with tf.GradientTape() as actor_tape:
            for idx, seq in enumerate(seqs):
                seq['reward'], mets1 = self.rewnorm[idx](rewards[idx])
                metrics.update(**{f'agent{idx}' + key: val for key, val in mets1.items()})
                target, mets2 = self.target(seq, idx)
                actor_loss_i, mets3 = self.actor_loss(seq, target, idx)
                metrics.update(**{f'agent{idx}' + key: val for key, val in mets2.items()})
                metrics.update(**{f'agent{idx}' + key: val for key, val in mets3.items()})
                actor_loss = actor_loss + actor_loss_i
                targets.append(target)
        with tf.GradientTape() as critic_tape:
            for idx, seq in enumerate(seqs):
                critic_loss_i, mets4 = self.critic_loss(seq, target, idx)
                critic_loss = critic_loss + critic_loss_i
                metrics.update(**{f'agent{idx}'+key: val for key, val in mets4.items()})
        actor_mets = self.actor_opt(actor_tape, actor_loss, self.actor)
        critic_mets = self.critic_opt(critic_tape, critic_loss, self.critic)
        metrics.update(**actor_mets, **critic_mets)
        self.update_slow_targets()
        return metrics
    ...
    def actor_loss(self, seq, target, idx):
        ...
        if self.config.actor_grad == 'dynamics':
            ...
            baseline = self._target_critic(seq['feat'][:-2], idx).mode()
            advantage = tf.stop_gradient(target[1:] - baseline)
            objective = policy.log_prob(seq['action'][1:-1]) * advantage
        elif self.config.actor_grad == 'both':
            baseline = self._target_critic[idx](seq['feat'][:-2]).mode()
    ...
    def critic_loss(self, seq, target, idx):
        ...
        # Targets:    [t0]  [t1]  [t2]
        # Loss:        l0    l1    l2
        dist = self.critic(seq['feat'][:-1], idx)
        target = tf.stop_gradient(target)
        weight = tf.stop_gradient(seq['weight'])
        critic_loss = -(dist.log_prob(target) * weight[:-1]).mean()
        metrics = {'critic': dist.mode().mean()}
    ...
    def target(self, seq, idx):
        ...
        reward = tf.cast(seq['reward'], tf.float32)
        disc = tf.cast(seq['discount'], tf.float32)
        value = self._target_critic(seq['feat'], idx).mode()
        # Skipping last time step because it is used for bootstrapping.
        target = common.lambda_return(reward[:-1], value[:-1], disc[:-1], bootstrap=value[-1], lambda_=self.config.discount_lambda, axis=0)
        metrics = {'critic_slow': value.mean(), 'critic_target': target.mean()}
        return target, metrics

    def update_slow_targets(self):
        if self.config.slow_target:
            if self._updates % self.config.slow_target_update == 0:
                mix = 1.0 if self._updates == 0 else float(self.config.slow_target_fraction)
                for s, d in zip(self.critic.variables, self._target_critic.variables):
